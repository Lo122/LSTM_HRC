...
saved to --output's .npz if given (keys: world_root_xyz, world_skeleton,
timestamps -- see main()'s end for the full list).
...
Usage, webcam:
    python pose_detection_live.py --source 0 --device cuda:0 `
        --user-height-m 1.75
...
calibrating iphone_intrinsics.json/iphone_extrinsics.json):
    python pose_detection_live.py --source iphone --dev-idx 0 `
        --capture-rotate90 90 --device cuda:0
...
CALIB_DATA_DIR = Path(__file__).resolve().parent.parent / "dataset" / "calib_data"
DEFAULT_YOLO_MODEL = Path(__file__).resolve().parent.parent / "dataset" / "model" / "yolo26m-pose.pt"
