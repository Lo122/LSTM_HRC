...
def normalize_keypoints(kpts_tensor):
    ...
    dist = torch.norm(kpts_preprocessed, dim=1)  # Get the mean distance from the center
...
def extract_posture_features(kpts_tensor):
    ...
    try:
        ...
        dist_shoulders = torch.norm(kpts_tensor[5] - kpts_tensor[6]) + 1e-6
        ...
        return torch.zeros(8), torch.zeros(1)
...
def run_pose_extraction(VIDEO_PATH):
    ...
    all_landmarks = torch.stack([f["norm_kpts"] for f in frames_out])
    all_degrees = torch.stack([f["features_degree"] for f in frames_out])
    all_ratios = torch.stack([f["features_ratio"] for f in frames_out])
